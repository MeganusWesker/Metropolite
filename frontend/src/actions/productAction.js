...
export const createProduct = (name, price, category, description, stock,images)=> async(dispatch)=>{
    try {
        ...
        const {data} = await axios.post(`/api/v1/admin/product/new`,{name, price, category, description, stock,images},config);
...
export const sendInquiryFunc = (email,queryTextarea,countryCode,role,myImages) => async (dispatch)=>{
   try {

    dispatch({type: SEND_INQUIRY_REQUEST});

    const config = {header:{"Content-Type":"application/json"}}; 

   console.log(email,queryTextarea,countryCode,role,myImages)


    const {data} = await axios.post(`/api/v1/sendinquiry`,{email,queryTextarea,countryCode,role,myImages},config);
